...
echo -e "\033[0;33mINPUT REQUIRED:\033[0m Do you want to create easy access shortcuts for folders? (y/n)" && read -r choice

while [[ ! $choice =~ ^[YyNn]$ ]]; do
    echo -e "\033[0;31mERROR:\033[0m Invalid input; please enter y or n" && read -r choice
done
...
if [ "$choice" = "Y" ] || [ "$choice" = "y" ]; then
    ...
    mkdir -p ${HOME}/.config && mv .config ${bin_folder} && ln -s ${bin_folder}/.config ${HOME}/.config
    mkdir -p ${HOME}/.cache && mv .cache ${bin_folder} && ln -s ${bin_folder}/.cache ${HOME}/.cache
    mkdir -p ${HOME}/.local && mv .local ${bin_folder} && ln -s ${bin_folder}/.local ${HOME}/.local
    mkdir -p ${HOME}/ondemand && mv ondemand ${bin_folder} && ln -s ${bin_folder}/ondemand ${HOME}/ondemand
fi

echo -e "\033[0;33mINPUT REQUIRED:\033[0m Do you want to install miniconda3? (y/n)" && read -r choice
...
echo -e "\033[0;33mINPUT REQUIRED:\033[0m Do you want to create a reticulate environment for R? (y/n)" && read -r choice
...
echo -e "\033[0;33mINPUT REQUIRED:\033[0m Do you want to correct OnDemand for RStudio? (y/n)" && read -r choice
...
if [ "$choice" = "Y" ] || [ "$choice" = "y" ]; then
    ...
    echo -e "\033[0;31mERROR:\033[0m LD_LIBRARY_PATH line not found in script.sh"
    fi
else
   echo "Exiting"
fi
